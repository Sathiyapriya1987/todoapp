...
app.get('/tasks', async (req,res) => {
    try {
        ...
        if (sortBy === "dueDate") {
            sortOption = { dueDate: 1 }; // Adcending  
...
// Create a new task and it to the array  ↓
...
app.patch('/tasks/complete/:id', async (req, res) => {
    try {
        ...
        res.json({ task: completedTask, message: "Task set to not complete!" });
...
app.delete('/tasks/delete/:id', async (req, res) => {
    ...
    app.put('/tasks/update/:id', async (req, res) => {
        try {
            ...
            const taskData = { title, description,dueDate };
            const updateTask = await Task.findByIdAndUpdate(taskId, taskData, { new: true });

            if (!updatedTask) {
                return res.status(404).json({ message: "Task not found!" });
            }

            res.jso({ task: updatedTask, message: "Task updated successfully!" });
