...
// -----------------Get tasks -----------------
        ...
        function formatTask(task) {
           ...
           `
             <div class="d-flex justify-content-between align-items-start">
                ...
                <button data-id="${task._id}" type="button" class="btn-close delete" aria-lable="Close"></button>
            </div>
            ...
            <div class="d-flex justify-content-between align-items-end">
                <div>
                 ...
                 <p class="m-0 text-decoration-line-through opacity-50"><strong>Created on: </strong>${new Date(task.createOn).toLocaleDateString()}</p>
            </div>
           `
           : 
           ...
           `
            ...
            <div class="d-flex justify-content-between align-items-end">
                ...
                <p class="m-0"><strong>Created on: </strong>${new Date(task.createOn).toLocaleDateString()}</p>
            </div>
           `;
           return li;
        }
 ...
 // ------------ Create tasks -------------
   async function createNewTask() {
        try {
            ...
            if (!response.ok) {
                throw new Error(`Failed to creat task!: ${response.status}`);
            }
 ...
 async function completeTask(taskId) {
    try {
        ...
        if (!response.ok) {
            throw new Error(`Failed to completed the task: $response.ststus}`);
        }
